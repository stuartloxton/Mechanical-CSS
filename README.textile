...
Mechanical CSS is an extension the CSS language which allows for a more modular and cleaner set of CSS files, MCSS currently runs through a PHP program which converts custom syntax enhancements into valid CSS. Currently the language allows for server-side imports, custom rule defining, variables and automatic compression via flags.

h3. Server-Side Imports

Server side imports are done by writing the valid CSS expression: <code>@import "file.mcss";</code> if the file extension is MCSS (Mechanical CSS) then the include is done on the server, otherwise the import is left for the browser to handle. You can always just rename css files to .mcss if you want them included on the server.
